...
from constants import IMG_TOKEN, SEP_TOKEN, RET_TOKEN, IGNORE_INDEX
from models.projectors import CONNECTOR_REGISTRY, RETRIEVAL_PROJECTOR_REGISTRY
...
class FrozenArgs:
    ...
    use_lora_on_lm: bool = field(default=False, metadata={"help": "Whether to use LoRA on the language model."})
    lm_lora_rank: int = field(default=8, metadata={"help": "The rank of the LoRA on the language model."})
    lm_lora_alpha: int = field(default=32, metadata={"help": "The alpha of the LoRA on the language model."})
    lm_lora_dropout: float = field(default=0.03, metadata={"help": "The dropout probability for the LoRA on the language model."})
...
class VIGIAConfig(PretrainedConfig):
    ...
    def __init__(self,
                ...
                use_lora_on_lm: bool = False,
                lm_lora_rank: int = 8,
                lm_lora_alpha: int = 32,
                lm_lora_dropout: float = 0.03,
                **kwargs):
        ...
        self.use_lora_on_lm = use_lora_on_lm
        self.lm_lora_rank = lm_lora_rank
        self.lm_lora_alpha = lm_lora_alpha
        self.lm_lora_dropout = lm_lora_dropout
...
class VIGIAModel(nn.Module):
    def __init__(self, config: VIGIAConfig): # Removed args
        ...
        if config.use_lora_on_lm:
            print("Using LoRA on the language model.")
            self.make_lm_lora(lora_rank=config.lm_lora_rank, lora_alpha=config.lm_lora_alpha, lora_dropout=config.lm_lora_dropout)
    ...
    def make_lm_lora(self, lora_rank: int = 8, lora_alpha: int = 32, lora_dropout: float = 0.03):
        ...
        # update the config
        self.config.use_lora_on_lm = True
        self.config.lm_lora_rank = lora_rank
        self.config.lm_lora_alpha = lora_alpha
        self.config.lm_lora_dropout = lora_dropout
    ...
    def merge_lm_lora(self):
        if isinstance(self.lm, LoraModel) or isinstance(self.lm, PeftModel):
            ...
            # update the config
            self.config.use_lora_on_lm = False
    ...
    def generate(self, embeddings=torch.FloatTensor, max_len: int = 32,
        ...
        with torch.no_grad():  # no tracking history
            ...
            for i in range(max_len):
                ...
                elif (self.eos_token_id and (next_token == self.eos_token_id).all()):
                    # End of generation.
                    break
...
class VIGIA(PreTrainedModel):
    ...
    def __call__(self, pixel_values: Tensor, input_ids: Optional[Tensor] = None, labels: Optional[Tensor] = None,
        ...
        if generate:
            return self.model.generate(pixel_values, num_words, temperature=temperature, top_p=top_p,
    ...
    def unfreeze_lm(self):
        if isinstance(self.model.lm, LoraModel) or isinstance(self.model.lm, PeftModel):
...
def load_mmplanllm(model_dir: str, run_name='pretrained_ckpt.pth.tar') -> VIGIA:
