...
from transformers import AutoTokenizer, AutoFeatureExtractor, AutoProcessor, AutoModelForImageTextToText
...
def get_args():
    ...
    parser.add_argument("--ckpt-path", type=str, default="/home/dmgcsilva/project/DATA/hub/models--OpenGVLab--InternVL3_5-8B-HF/snapshots/741a7d03020411e666c6109218ab71e08151ef86/", help="Path to the checkpoint file")
...
def load_model_and_tokenizer(args):
    ...
    model = AutoModelForImageTextToText.from_pretrained(args.ckpt_path, torch_dtype=torch.float16)
...
def generate_response(model, tokenizer, task, history, images):
    ...
    generate_ids = model.generate(**item, max_new_tokens=128, do_sample=False, pad_token_id=tokenizer.tokenizer.eos_token_id)
...
if __name__ == "__main__":
    ...
    out_path = "/home/dmgcsilva/workbench/followup/code/inference/results/internvl35_gen_dialogues.json"
    with open(out_path, "w") as f:
        json.dump(generated_dialogues, f, indent=4)

    print(f"Generated dialogues saved to {out_path}")
# CUDA_VISIBLE_DEVICES=0 HF_HUB_OFFLINE=1 python -m inference.internvl35.dialogue_sim --device_id 0