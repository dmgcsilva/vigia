...
FILE_PATH = "/home/dmgcsilva/project/DATA/mmplanllm/conversations_eval_cvmr.json"
...
def inference():
    ...
    if not os.path.exists(args.ckpt_path):
        output_file = f"./{output_file}_eval_cvmr.json"
    else:
        output_file = os.path.join(args.ckpt_path, f'{output_file}_eval_cvmr.json')
    ...
    with torch.inference_mode():
        ...
        for sim in [1.0, 0.95, 0.9, 0.85, 0.8, 0.75, 0.7, 0.65, 0.6, 0.55, 0.5, 0.45, 0.4, 0.35, 0.3, 0.25, 0.2, 0.15, 0.1, 0.05]:
            metrics_inner = {}
            for k in [1, 5, 10]:
                for m in [0.5, 0.7, 1.0]:
