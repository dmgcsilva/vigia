...
from transformers import AutoTokenizer, AutoFeatureExtractor, AutoProcessor, LlavaOnevisionForConditionalGeneration
...
def get_args():
    ...
    parser.add_argument("--ckpt-path", type=str, default="llava-hf/llava-onevision-qwen2-7b-ov-hf", help="Path to the checkpoint file")
...
def load_model_and_tokenizer(args):
    ...
    model = LlavaOnevisionForConditionalGeneration.from_pretrained(args.ckpt_path, torch_dtype=torch.float16)
...
def generate_response(model, tokenizer, task, history, images):
    ...
    generate_ids = model.generate(**item, max_new_tokens=256, do_sample=False, pad_token_id=tokenizer.tokenizer.eos_token_id)
...
def build_inputs(tokenizer, task, history, images):
    ...
    # print(f"Input prompt: {tokenizer.decode(inputs['input_ids'][0], skip_special_tokens=False)}")
...
if __name__ == "__main__":
    ...
    out_path = "/home/dmgcsilva/workbench/followup/code/inference/results/llava_onevision_gen_dialogues.json"
    with open(out_path, "w") as f:
        json.dump(generated_dialogues, f, indent=4)

    print(f"Generated dialogues saved to {out_path}")
# CUDA_VISIBLE_DEVICES=0 HF_HUB_OFFLINE=1 python -m inference.llava_onevision.dialogue_sim --device_id 0