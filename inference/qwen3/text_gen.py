...
FILE_PATH = "/home/dmgcsilva/project/DATA/mmplanllm/conversations_test_text.json"


def get_args():
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument("--model", type=str, default="Qwen/Qwen3-VL-8B-Instruct", help="Model name or path")
...
def inference():
    ...
    output_file = 'inference/results/{name}_textgen.json'
    name = "qwen3"
    ...
    with torch.inference_mode():
        ...
        model = Qwen3VLForConditionalGeneration.from_pretrained(args.model, torch_dtype=torch.float16)
        ...
        with torch.no_grad():
            for idx in tqdm(range(max_samples), desc="Inference"):
                ...
                generate_ids = model.generate(**inputs, max_new_tokens=128, do_sample=False, pad_token_id=processor.tokenizer.pad_token_id)
        ...
        # save predictions
        output_dict = {}
        clean_predictions = [p.replace(s, "").strip().replace("**", "").strip() for p, s in zip(predictions, test_dataset.raw_sources)]
        ...
        metrics = dict()
        if os.path.exists(output_file.replace('_textgen.json', '_metrics.json')):
            with open(output_file.replace('_textgen.json', '_metrics.json'), 'r') as f:
                metrics = json.load(f)
        ...
        metrics['textgen'] = cap_scores


        print(metrics)
        with open(output_file.replace('_textgen.json', '_metrics.json'), 'w') as f:
